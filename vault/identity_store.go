...
func NewIdentityStore(ctx context.Context, core *Core, config *logical.BackendConfig, logger log.Logger) (*IdentityStore, error) {
	...
	iStore.oidcCache, err = newOIDCCache(nil, ns.ID, "oidc", cache.NoExpiration, logger)
	if err != nil {
		return nil, err
	}
	iStore.oidcAuthCodeCache, err = newOIDCCache(nil, ns.ID, "oidc-auth", 5*time.Minute, logger)
	if err != nil {
		return nil, err
	}
